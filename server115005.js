...
io.to(getGroupRoomName(groupId)).emit('groupCallUserJoined', {
  ...
  // Stage pinning (teacher/starter can focus a participant on everyone’s UI)
  socket.on('groupCallPin', (data) => {
    try {
      if (!socket.userId) return;
      const groupId = String(data?.groupId || '');
      const callId = String(data?.callId || '');
      const pinnedUserId = data?.pinnedUserId ? String(data.pinnedUserId) : null;
      if (!groupId || !callId) return;

      const call = activeGroupCalls.get(groupId);
      if (!call || String(call.callId) !== callId) return;

      // Only starter can pin (simple + safe)
      if (String(call.startedBy) !== String(socket.userId)) {
        return socket.emit('groupCallError', { error: 'Only starter can pin' });
      }

      // Pin must be a current participant (or null)
      if (pinnedUserId && !call.participants.has(pinnedUserId)) return;

      call.pinnedUserId = pinnedUserId;
      activeGroupCalls.set(groupId, call);

      io.to(getGroupRoomName(groupId)).emit('groupCallPinned', {
        groupId,
        callId,
        pinnedUserId
      });
