...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ userId: user._id, username: user.username, role: user.role || (user.isAdmin ? 'admin' : 'student'), isAdmin: !!user.isAdmin }, process.env.JWT_SECRET, { expiresIn: '30d' });
...
app.get('/api/me', authenticateToken, async (req, res) => {
  try {
    ...
    // Backward compatibility: derive role if missing
    if (!safeUser.role) safeUser.role = safeUser.isAdmin ? 'admin' : 'student';
...
// ==================== PET + COINS API ====================
// (admin guard above)
f (!user || !user.isAdmin) return res.status(403).json({ error: 'Admin required' });
    req.adminUser = user;
    next();
  } catch (e) {
