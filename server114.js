...
// Admin manages allowed categories for channels. We store both "name" (Uzbek label) and "slug" (value stored on Channel.category).
const ChannelCategoryCatalogSchema = new mongoose.Schema({
  name: { type: String, required: true, trim: true },
  slug: { type: String, required: true, unique: true, trim: true, index: true },
  createdAt: { type: Date, default: Date.now }
});
const ChannelCategoryCatalog = mongoose.models.ChannelCategoryCatalog || mongoose.model('ChannelCategoryCatalog', ChannelCategoryCatalogSchema);

function slugifyUzSafe(s){
  return String(s||'').trim().toLowerCase()
    .replace(/['"’`]/g,'')
    .replace(/\s+/g,'-')
    .replace(/[^a-z0-9\-а-яёқғҳўӣҷ]+/gi,'')
    .replace(/\-\-+/g,'-')
    .replace(/^\-+|\-+$/g,'');
  // Seed default channel categories (if empty) — Uzbek labels + safe slugs
  const catCount = await ChannelCategoryCatalog.countDocuments({});
  if (catCount === 0) {
    const defaultCats = [
      { name: "E'lonlar", slug: "elonlar" },
      { name: "Ta'lim", slug: "talim" },
      { name: "Tadbirlar", slug: "tadbirlar" },
      { name: "Sport", slug: "sport" },
      { name: "Madaniyat", slug: "madaniyat" },
      { name: "Boshqa", slug: "boshqa" }
    ];
    try { await ChannelCategoryCatalog.insertMany(defaultCats, { ordered: false }); } catch(_) {}
  }
...
// Validate university against admin catalog (if catalog has entries)
    const uniVal = String(university || '').trim();
    const uniCount = await UniversityCatalog.countDocuments({});
    if (uniCount > 0 && uniVal) {
      const uniOk = await UniversityCatalog.findOne({ name: uniVal }).lean();
      if (!uniOk) return res.status(400).json({ error: "Universitet ro'yxatda yo'q (admin qo'shganini tanlang)" });
    }
    // Validate category against admin catalog (if catalog has entries)
    const catVal = String(category || '').trim();
    const catCount = await ChannelCategoryCatalog.countDocuments({});
    if (catCount > 0) {
      const catOk = await ChannelCategoryCatalog.findOne({ $or: [{ slug: catVal }, { name: category }] }).lean();
      if (!catOk) return res.status(400).json({ error: "Kategoriya ro'yxatda yo'q (admin qo'shganini tanlang)" });
    }
...
// Public: channel categories list (used by channels/channel pages)
app.get('/api/catalog/channel-categories', async (req, res) => {
  try {
    const list = await ChannelCategoryCatalog.find({}).sort({ name: 1 }).lean();
    res.json({ success: true, categories: list.map(c => ({ name: c.name, slug: c.slug })) });
  } catch (e) {
...
// Admin: manage channel categories catalog
app.post('/api/admin/catalog/channel-categories', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const name = String(req.body.name || '').trim();
    const slugIn = String(req.body.slug || '').trim();
    const slug = slugIn ? slugifyUzSafe(slugIn) : slugifyUzSafe(name);
    if (!name) return res.status(400).json({ error: 'name required' });
    if (!slug) return res.status(400).json({ error: 'slug required' });
    await ChannelCategoryCatalog.updateOne(
      { slug },
      { $set: { name, slug } },
      { upsert: true }
    );
...
app.delete('/api/admin/catalog/channel-categories/:slug', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const slug = String(req.params.slug || '').trim();
    if (!slug) return res.status(400).json({ error: 'slug required' });
    await ChannelCategoryCatalog.deleteOne({ slug });
    res.json({ success: true });
  } catch (e) {
