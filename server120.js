...
(() => {
  ...
  function downloadLiveKitClient(cb) {
    const url = 'https://unpkg.com/livekit-client/dist/livekit-client.umd.min.js';
    const tmp = vendorFile + '.tmp';
    const file = fs.createWriteStream(tmp);
    https.get(url, (resp) => {
      if (resp.statusCode !== 200) {
        file.close(() => { try { fs.unlinkSync(tmp); } catch(_){} });
        return cb(new Error('Download failed: ' + resp.statusCode));
      }
      resp.pipe(file);
      file.on('finish', () => file.close(() => {
        try { fs.renameSync(tmp, vendorFile); } catch (e) { return cb(e); }
        cb(null);
      }));
    }).on('error', (err) => {
      file.close(() => { try { fs.unlinkSync(tmp); } catch(_){} });
...
io.to(getGroupRoomName(groupId)).emit('groupCallUserJoined', {
  ...
  socket.on('groupCallEnd', async (data) => {
    try {
      ...
      activeGroupCalls.delete(gro
      // Mark lesson ended (if any)
      try {
        const lesson = await GroupLesson.findOne({ groupId, callId, status: 'live' }).select('_id startedAt').lean();
        if (lesson) {
          const endedAt = new Date();
          await GroupLesson.updateOne({ _id: lesson._id }, { $set: { status: 'ended', endedAt } }).catch(()=>{});
          // Close attendance for anyone still "in call"
          await GroupAttendance.updateMany(
            { lessonId: lesson._id, leftAt: null },
            [{ $set: { leftAt: endedAt, durationSec: { $round: [{ $divide: [{ $subtract: [endedAt, '$joinedAt'] }, 1000] }, 0] } } }]
          ).catch(()=>{});
        }
      } catch(e) { console.warn('GroupLesson end update failed:', e?.message || e); }
upId);
      io.to(getGroupRoomName(groupId)).emit('groupCallEnded', {
        groupId,
        callId,
