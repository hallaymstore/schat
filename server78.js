...
socket.on('live:hostJoin', async ({ liveId }) => {
  try {
    ...
    if (live.status !== 'live') return socket.emit('live:error', { error: 'Live is not started' });
...
app.post('/api/lives', authenticateToken, async (req, res) => {
  try {
    ...
    const { title, description, startAt, courseId, type, price } = req.body || {};
...
app.put('/api/lives/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const { title, description, startAt, type, price, status } = req.body || {};
...
app.post('/api/lives/:id/start', authenticateToken, async (req, res) => {
  try {
    ...
    if (io) io.emit('live:status', { liveId: String(live._id), status: 'live', startedAt: live.startedAt });
    res.json({ success: true, live: live.toObject() });
  } catch (e) {
...
app.post('/api/lives/:id/stop', authenticateToken, async (req, res) => {
  try {
    ...
    if (io) io.emit('live:status', { liveId: String(live._id), status: 'ended', endedAt: live.endedAt });
    res.json({ success: true, live: live.toObject() });
  } catch (e) {
