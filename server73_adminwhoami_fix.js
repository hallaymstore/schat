...
async function requireAdmin(req, res, next) {
  try {
    // Prefer explicit flags in DB
    const user = await User.findById(req.userId).select('username isAdmin role').lean();
    if (!user) return res.status(404).json({ error: 'User not found' });

    const role = String(user.role || '').toLowerCase();
    const isAdminFlag = !!user.isAdmin || role === 'admin';

    // Fallback: allow env-based admin usernames too (for legacy configs)
    const admins = (process.env.ADMIN_USERNAMES || '')
      .split(',')
      .map(s => s.trim())
      .filter(Boolean);

    const isEnvAdmin = admins.length ? admins.includes(user.username) : false;

    if (!isAdminFlag && !isEnvAdmin) {
...
// Admin whoami (frontend admin dashboard health-check)
