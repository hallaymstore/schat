...
// Post recording for group lesson (host teacher only)
app.post('/api/group-lessons/:lessonId/recording', authenticateToken, upload.single('recording'), async (req, res) => {
  try {
    const lessonId = String(req.params.lessonId || '');
    const incomingTitle = String(req.body?.title || '').trim();
    
    if (!lessonId || !req.file) return res.status(400).json({ error: 'lessonId and video file required' });

    const lesson = await GroupLesson.findById(lessonId).select('hostId groupId').lean();
    if (!lesson) return res.status(404).json({ error: 'Lesson not found' });
    if (String(lesson.hostId) !== String(req.userId)) return res.status(403).json({ error: 'Only host can upload recording' });

    const uploadsDir = path.join(__dirname, 'uploads', 'lessons');
    try { require('fs').mkdirSync(uploadsDir, { recursive: true }); } catch(_) {}

    const tmpPath = req.file.path;
    async function saveLocalRecordingFile(srcPath) {
      const fname = `lesson_${lessonId}_${Date.now()}${path.extname(srcPath) || '.webm'}`;
      const fp = path.join(uploadsDir, fname);
      await require('fs').promises.rename(srcPath, fp).catch(async () => {
        await require('fs').promises.copyFile(srcPath, fp);
        await require('fs').promises.unlink(srcPath).catch(()=>{});
      });
      const url = `/uploads/lessons/${fname}`;
      const st = await require('fs').promises.stat(fp).catch(()=>({ size: 0 }));
      return { secure_url: url, url, public_id: '', bytes: st.size || 0, duration: 0, _local: true };
    }
    ...
    if (hasCloudinary) {
      ...
      try {
        ...
        console.warn('Cloudinary upload_large failed, fallback to local:', cloudErr?.message || cloudErr);
...
// Teacher-only helpers (whiteboard + stage pin). Relayed via socket.io room.
socket.on('groupWhiteboard:stroke', (payload) => {
  try {
    if (!socket.userId) return;
    const groupId = String(payload?.groupId || '');
    const callId = String(payload?.callId || '');
    ...
    const call = activeGroupCalls.get(groupId);
    if (!call || String(call.callId) !== callId) return;
    // Relay to group room (including teacher for multi-device sync)
    io.to(getGroupRoomName(groupId)).emit('groupWhiteboard:stroke', Object.assign({}, payload, {
      from: String(socket.userId),
      ts: Date.now()
    }));
  } catch(e) {
    console.error('groupWhiteboard:stroke error', e);
  }
});

socket.on('groupWhiteboard:clear', (payload) => {
  try {
    if (!socket.userId) return;
    const groupId = String(payload?.groupId || '');
    const callId = String(payload?.callId || '');
    ...
    const call = activeGroupCalls.get(groupId);
    if (!call || String(call.callId) !== callId) return;
    io.to(getGroupRoomName(groupId)).emit('groupWhiteboard:clear', {
      groupId, callId, from: String(socket.userId), ts: Date.now()
    });
  } catch(e) {
    console.error('groupWhiteboard:clear error', e);
  }
});
socket.on('groupWhiteboard:toggle', (payload) => {
  try {
    if (!socket.userId) return;
    const groupId = String(payload?.groupId || '');
    const callId = String(payload?.callId || '');
    ...
    const call = activeGroupCalls.get(groupId);
    if (!call || String(call.callId) !== callId) return;
    io.to(getGroupRoomName(groupId)).emit('groupWhiteboard:toggle', {
      groupId, callId, enabled, from: String(socket.userId), ts: Date.now()
    });
  } catch(e) {
    console.error('groupWhiteboard:toggle error', e);
  }
});

// Stage pin: teacher selects a student to show on stage (everyone updates layout).
socket.on('groupStage:pin', (payload) => {
  try {
    if (!socket.userId) return;
    const groupId = String(payload?.groupId || '');
    const callId = String(payload?.callId || '');
    const pinnedUserId = payload?.pinnedUserId ? String(payload.pinnedUserId) : '';
    if (!groupId || !callId) return;
    const call = activeGroupCalls.get(groupId);
    if (!call || String(call.callId) !== callId) return;
    io.to(getGroupRoomName(groupId)).emit('groupStage:pin', {
      groupId, callId, pinnedUserId, from: String(socket.userId), ts: Date.now()
    });
  } catch(e) {
    console.error('groupStage:pin error', e);
  }
});
