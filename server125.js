...
const { AccessToken } = require('livekit-server-sdk'); // LiveKit SFU tokens
...
const io = socketIO(server, {
  cors: {
    origin: ['http://localhost:3000', 'https://schat-q1nj.onrender.com', 'https://students.hallaym.site', 'https://schat.hallaym.site'],
    credentials: true
  },
...
// ==================== LiveKit Token (SFU) ====================
// Requires env:
//   LIVEKIT_URL=https://<your-livekit-host>  (or wss://...)
//   LIVEKIT_API_KEY=...
//   LIVEKIT_API_SECRET=...
// Client (browser) calls POST /api/livekit/token { roomName, canPublish, canSubscribe, isSpeaker }
// and receives { token, url }.
app.post('/api/livekit/token', authenticateToken, attachUserRole, async (req, res) => {
  try {
    const roomName = String(req.body?.roomName || req.body?.room || req.query?.roomName || req.query?.room || '').trim();
    ...
    if (!url || !apiKey || !apiSecret) {
      return res.status(500).json({ error: 'LiveKit is not configured on server (LIVEKIT_URL/API_KEY/API_SECRET)' });
    }

    // Identity: stable per user
    const identity = String(req.userId || '').trim() || ('anon_' + Date.now());
    const name = String(req.userRole || '').toLowerCase() === 'teacher' ? 'Teacher' : 'Student';

    const canPublish = !!(req.body?.canPublish);
    const canSubscribe = (req.body?.canSubscribe !== false); // default true

    // In LiveKit, publish/subscribe are controlled by grants.
    ...
    at.addGrant({
      ...
      // keep it permissive; server-side moderation can be added later
      canPublishData: true
    });
...
/* LiveKit token endpoint is ENABLED below (search: /api/livekit/token).
...
// ==================== LiveKit client UMD local cache ====================
// Fix: CDN blocked/unstable => browser couldn't load livekit-client and fell back to mesh.
// This endpoint downloads the UMD bundle once and serves it from /public/vendor with correct MIME.
// URL used by pages: /vendor/livekit-client.umd.min.js
(() => {
  const fs = require('fs');
  const https = require('https');
  const vendorDir = path.join(__dirname, 'public', 'vendor');
  const vendorFile = path.join(vendorDir, 'livekit-client.umd.min.js');
  try { fs.mkdirSync(vendorDir, { recursive: true }); } catch (_) {}

  function downloadLiveKitClient(cb) {
    // NOTE:
    // - unpkg often replies with 302 redirect to a versioned URL.
    // - previous implementation treated 302 as a hard error.
    // Fix: use fetch with redirect-follow and multiple CDN fallbacks.

    const candidates = [
      // cdnjs (Cloudflare) is very stable and includes UMD bundles
      'https://cdnjs.cloudflare.com/ajax/libs/livekit-client/2.15.7/livekit-client.umd.min.js',
    ...
    async function tryOne(url) {
      const resp = await fetch(url, {
        redirect: 'follow',
        headers: {
          'User-Agent': 'schat-livekit-client-cache/1.0',
          'Accept': 'application/javascript,text/javascript,*/*'
        }
      });
      if (!resp.ok) throw new Error(`HTTP ${resp.status} for ${url}`);
      const buf = Buffer.from(await resp.arrayBuffer());
      if (buf.length < 10_000) throw new Error(`Downloaded file too small (${buf.length} bytes) from ${url}`);
      require('fs').writeFileSync(tmp, buf);
      require('fs').renameSync(tmp, vendorFile);
    }

    (async () => {
      try {
        const fs2 = require('fs');
        try { if (fs2.existsSync(tmp)) fs2.unlinkSync(tmp); } catch (_) {}
        ...
        cb(lastErr || new Error('All CDN candidates failed'));
      } catch (e) {
  ...
  app.get('/vendor/livekit-client.umd.min.js', (req, res) => {
    try {
      if (fs.existsSync(vendorFile) && fs.statSync(vendorFile).size > 10_000) {
        res.setHeader('Content-Type', 'application/javascript; charset=utf-8');
        res.setHeader('Cache-Control', 'public, max-age=86400');
        return res.sendFile(vendorFile);
      }
      // Not cached yet: download then serve
      downloadLiveKitClient((err) => {
        if (err) {
          console.error('❌ livekit-client download error:', err);
          return res.status(502).type('text/plain').send('Failed to fetch livekit-client. Check server internet access.');
        }
        res.setHeader('Content-Type', 'application/javascript; charset=utf-8');
        res.setHeader('Cache-Control', 'public, max-age=86400');
        return res.sendFile(vendorFile);
      ...
      console.error('❌ /vendor/livekit-client.umd.min.js error:', e);
      return res.status(500).type('text/plain').send('Internal error serving livekit-client');
    }
  });
})();
...
// --- Range-safe streaming for lesson recordings (prevents RangeNotSatisfiable / black screen) ---
app.get('/uploads/lessons/:file', (req, res) => {
  try {
    const fs = require('fs');
    const file = String(req.params.file || '');
    const filePath = path.join(__dirname, 'uploads', 'lessons', file);
    ...
    const contentType = 'video/webm'; // our recorder outputs webm by default
    ...
    // invalid range -> 416 (do not throw)
...
// Download proxy for local lesson recordings (device download)
app.get('/api/group-lessons/download', authenticateToken, async (req, res) => {
  try {
    const fs = require('fs');
    const u = String(req.query.url || '');
    if (!u.startsWith('/uploads/lessons/')) return res.status(400).json({ error: 'Invalid url' });
...
// Client flow:
//   POST /api/livekit/token { room, canPublish, canSubscribe, isSpeaker }
//   -> { url, token, room, identity, name }
app.post('/api/livekit/token', authenticateToken, async (req, res) => {
  try {
    ...
    const apiSecret = String(process.env.LIVEKIT_API_SECRET || '').trim();
    if (!url || !apiKey || !apiSecret) {
      return res.status(500).json({ error: 'LiveKit env missing', code: 'LIVEKIT_ENV_MISSING' });
    }

    const body = req.body || {};
    const room = String(body.room || body.roomName || '').trim();
    if (!room) return res.status(400).json({ error: 'room required' });

    const canPublish = body.canPublish === true; // default false
    const canSubscribe = (body.canSubscribe !== false); // default true
    const isSpeaker = body.isSpeaker === true;

    // Identity must be stable and unique across connections
    const identity = String(req.userId);
    const u = await User.findById(req.userId).select('fullName username role').lean();
    ...
    at.addGrant({
      room,
      roomJoin: true,
      canPublish: !!canPublish || !!isSpeaker,
      canSubscribe: !!canSubscribe,
      canPublishData: true
    });
...
// Lesson stage state (in-memory): lessonId -> { stageUserId, updatedAt }
const lessonStages = new Map();
function setLessonStage(lessonId, stageUserId) {
  if (!lessonId) return;
  lessonStages.set(String(lessonId), { stageUserId: String(stageUserId || ''), updatedAt: Date.now() });
}
function getLessonStage(lessonId) {
  const st = lessonStages.get(String(lessonId));
  return st ? String(st.stageUserId || '') : '';
}
...
// ==================== GROUP CALL (WebRTC Mesh Signaling) ====================
  ...
  // ==================== LESSON "RAISE HAND" + STAGE CONTROL ====================
  // Student -> teacher: request to speak
  socket.on('lessonRaiseHand', async (data) => {
    try {
      if (!socket.userId) return;
      const lessonId = String(data?.lessonId || '');
      if (!lessonId) return;
      ...
      const info = await User.findById(socket.userId).select('fullName username role').lean().catch(()=>null);
      const controllerId = getLessonController(lessonId) || String(lesson.hostId);

      // Send only to controller (teacher/admin) to avoid spam to everyone
      emitToUser(controllerId, 'lessonHandRaised', {
        lessonId,
        fromUserId: String(socket.userId),
        from: info ? { userId: String(info._id), fullName: info.fullName, username: info.username, role: info.role } : null,
        ts: Date.now()
      });
    } catch (e) {
      console.error('lessonRaiseHand error:', e);
    }
  });

  // Teacher/admin: set who is on main stage (teacherStage). Everyone's UI reorders.
  socket.on('lessonSetStage', async (data) => {
    try {
      if (!socket.userId) return;
      const lessonId = String(data?.lessonId || '');
      const stageUserId = String(data?.stageUserId || '');
      if (!lessonId || !stageUserId) return;
      ...
      if (!isAdmin && String(controllerId) !== String(socket.userId) && String(lesson.hostId) !== String(socket.userId)) {
        return socket.emit('lessonError', { error: 'Only teacher/controller can change stage' });
      }

      // Validate target is in call participants if possible
      setLessonStage(lessonId, stageUserId);

      // Broadcast to whole group room so all clients reorder tiles
      io.to(getGroupRoomName(String(lesson.groupId))).emit('lessonStageChanged', {
        lessonId,
        stageUserId: getLessonStage(lessonId),
        teacherId: String(lesson.hostId),
        controllerId: controllerId
      });
    } catch (e) {
      console.error('lessonSetStage error:', e);
      socket.emit('lessonError', { error: 'Failed to set stage' });
    }
  });
...
socket.on('groupCallStart', async (data) => {
    try {
      ...
      try {
        ...
        if (starter && ['teacher','admin'].includes(String(starter.role || '').toLowerCase())) {
          ...
          if (!getLessonStage(lessonId)) setLessonStage(lessonId, socket.userId);
          try { io.to(getGroupRoomName(String(groupId))).emit('lessonStageChanged', { lessonId, stageUserId: getLessonStage(lessonId) }); } catch(e){}
      ...
      socket.emit('groupCallStarted', {
        ...
        participantInfos,
        lessonId,
        stageUserId: lessonId ? getLessonStage(lessonId) : ''
      });
  ...
  socket.on('groupCallJoin', async (data) => {
    try {
      ...
      if (lesson && lesson._id) {
        if (lessonId && !getLessonStage(lessonId)) setLessonStage(lessonId, lesson.hostId);
...
io.to(getGroupRoomName(groupId)).emit('groupCallUserJoined', {
  ...
  socket.on('groupCallEnd', async (data) => {
    try {
      if (!socket.userId) return;
      const groupId = String(data?.groupId || '');
      const callId = String(data?.callId || '');
      ...
      // Mark lesson ended (if any)
      try {
        const lesson = await GroupLesson.findOne({ groupId, callId, status: 'live' })
        ...
        if (lesson && lesson._id) {
          ...
          // Close attendance for anyone still "in call"
          await GroupAttendance.updateMany(
            { lessonId: lesson._id, leftAt: null },
            [
              {
                $set: {
                  leftAt: endedAt,
        ...
        console.warn('GroupLesson end update failed:', e?.message || e);
      }
